...
/**
 * State machine representation for the Two-Factor Authentication (2FA) management page.
 *
 * @description
 * Discriminates between the various operational stages of 2FA configuration:
 * - `loading`: Fetching current 2FA status from the backend API.
 * - `disabled`: 2FA is not configured or active on the admin account.
 * - `setup`: In-progress setup flow presenting QR code, secret, and backup codes for verification.
 * - `enabled`: 2FA is active, showing status and the disable protection form.
 */
type Stage =
  | { kind: "loading" }
  | { kind: "disabled" }
...
/**
 * Administrator dashboard page component for managing Two-Factor Authentication (2FA).
 *
 * @description
 * Provides a complete client-side interface for configuring TOTP-based 2FA:
 * - Displays active 2FA status and remaining backup codes.
 * - Initiates 2FA setup with QR code display and secret key provisioning.
 * - Verifies initial 6-digit TOTP token to activate 2FA.
 * - Disables 2FA by verifying the administrator password alongside a TOTP or backup code.
 *
 * @returns {JSX.Element} The rendered 2FA management admin page.
 */
...
/**
 * Props for the {@link SetupPanel} component.
 *
 * @description
 * Defines callbacks and state required during the three-step 2FA enrollment workflow.
 */
interface SetupPanelProps {
  /** Current setup stage state containing provisioning payload, entered token, and acknowledgment status */
  stage: Extract<Stage, { kind: "setup" }>;
  /** Indicates whether an asynchronous verification or setup network request is in progress */
  busy: boolean;
  /** Callback invoked when the user modifies the 6-digit TOTP verification input */
  onChangeCode: (code: string) => void;
  /** Callback invoked when the user confirms they have copied and stored their recovery backup codes */
  onAcknowledgeCodes: () => void;
  /** Callback to copy sensitive setup credentials to the system clipboard */
  onCopy: (text: string, label: string) => void;
  /** Callback invoked to submit the entered 6-digit code for final verification and activation */
  onVerify: (code: string) => void;
  /** Callback invoked when canceling the setup process, reverting to the disabled stage */
  onCancel: () => void;
}

/**
 * Sub-component presenting the multi-step 2FA setup and verification workflow.
 *
 * @description
 * Guides the administrator through:
 * 1. Scanning a QR code or manually entering the secret TOTP key into an authenticator app.
 * 2. Reviewing and acknowledging single-use emergency backup recovery codes.
 * 3. Entering the first 6-digit TOTP code generated by the authenticator app to verify and activate 2FA.
 *
 * @param props - Component properties and callbacks defined by {@link SetupPanelProps}.
 * @param props.stage - Current setup stage state containing provisioning payload, entered token, and acknowledgment status.
 * @param props.busy - Indicates whether an asynchronous verification or setup network request is in progress.
 * @param props.onChangeCode - Callback invoked when the user modifies the 6-digit TOTP verification input.
 * @param props.onAcknowledgeCodes - Callback invoked when the user confirms they have copied and stored their recovery backup codes.
 * @param props.onCopy - Callback to copy sensitive setup credentials to the system clipboard.
 * @param props.onVerify - Callback invoked to submit the entered 6-digit code for final verification and activation.
 * @param props.onCancel - Callback invoked when canceling the setup process, reverting to the disabled stage.
 * @returns {JSX.Element} The rendered setup panel step container.
 */
function SetupPanel({
  stage,
  busy,
